...
let Matrix = class Matrix {
    ...
    parseData(data) {
        ...
        for (var _i = 0; _i < optionTables.length; _i++) {
            //Add Date Headers
            let a = moment(this.dataService.getCurrentBacktestDate(), "YYYY/MM/DD hh:mm:ss");
            let b = moment(dateKeys[_i]);
    ...
    positionDeSelect(evnt) {
        ...
        if (val != '') {
            ...
            let tempPosition = new OptionPosition;
...
Matrix = __decorate([
    ...
    __metadata("design:paramtypes", [HttpClient, DataService])
], Matrix);
export { Matrix };
